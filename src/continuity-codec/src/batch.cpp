// Continuity.Codec.Dhall - Batch Evaluator
// Evaluate all BUILD.dhall files in a project
//
...
// Parse and evaluate a single file (with import resolution)
bool process_file(Arena& arena, Interner& interner, ImportResolver& resolver, const fs::path& path,
                  std::chrono::microseconds& parse_time, std::chrono::microseconds& resolve_time,
...
int main(int argc, char** argv) {
  ...
  spdlog::info("Continuity.Codec.Dhall Batch Evaluator");
  ...
  Arena arena;
  Interner interner;
  ImportResolver resolver(arena, interner);
  ...
  for (const auto& file : files) {
    ...
    if (process_file(arena, interner, resolver, file, parse_time, resolve_time, eval_time)) {
      success_count++;
  ...
  spdlog::info("Import cache: {} files", resolver.cache_size());
