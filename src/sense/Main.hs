...
usage = do
    ...
    putStrLn "  stan <target>      Show Stan static analysis report"
    putStrLn ""
    putStrLn "Options:"
...
cmdStan :: [String] -> IO ()
cmdStan args = case args of
    [] -> do
        putStrLn "Usage: sense stan <target>"
        exitFailure
    (target : _) -> do
        let dir = targetDir target
        genIfNeeded dir
        -- Build the stan subtarget and display the report
        let stanTarget = target <> "[stan]"
        putStrLn $ "Running Stan analysis for " <> target
        callBuck2 ["build", stanTarget]
