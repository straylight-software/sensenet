...
-- Using Text.isSuffixOf instead of Data.List.isSuffixOf to avoid infinite list issues
    ...
    findPcFiles dir = do
      ...
      if exists
        then do
          ...
          pure [T.pack f | f <- contents, ".pc" `T.isSuffixOf` T.pack f]
        else pure []
...
detectBuildSystem attrs =
  case Map.lookup "buildPhase" attrs of
    ...
    Just _ -> BSPrebuilt
    Nothing -> case Map.lookup "builder" attrs of
      Just (Aeson.String b)
