...
ruleDeps = \case
  ...
  RLeanBinary r -> r.deps
  RLeanLibrary r -> r.deps
  RNvBinary r -> r.deps
  RNvLibrary r -> r.deps
  RPureScriptApp _ -> []
  RPureScriptBinary _ -> []
...
-- | Extract rule source files
ruleSrcs :: Rule -> [Text]
ruleSrcs = \case
  RCxxBinary r -> r.srcs
  RCxxLibrary r -> r.srcs ++ r.hdrs
  RRustBinary r -> r.srcs
  RRustLibrary r -> r.srcs
  RHaskellBinary r -> r.srcs
  RHaskellLibrary r -> r.srcs
  RHaskellFFIBinary r -> r.hsSrcs ++ r.cxxSrcs ++ r.cxxHeaders
  RLeanBinary r -> r.srcs
  RLeanLibrary r -> r.srcs
  RNvBinary r -> r.srcs
  RNvLibrary r -> r.srcs ++ r.exportedHeaders
...
-- | Extract rule kind (type name)
ruleKind :: Rule -> Text
ruleKind = \case
  RCxxBinary _ -> "cxx_binary"
  RCxxLibrary _ -> "cxx_library"
  RRustBinary _ -> "rust_binary"
  RRustLibrary _ -> "rust_library"
  RHaskellBinary _ -> "haskell_binary"
  RHaskellLibrary _ -> "haskell_library"
  RHaskellFFIBinary _ -> "haskell_ffi_binary"
  RLeanBinary _ -> "lean_binary"
  RLeanLibrary _ -> "lean_library"
  RNvBinary _ -> "nv_binary"
  RNvLibrary _ -> "nv_library"
