...
benchmark path iterations = do
  ...
  putStrLn $ show upstreamTime ++ " total, " ++ 
             show (realToFrac upstreamTime / fromIntegral iterations :: Double) ++ "s/iter"
  ...
  putStrLn $ show fastTime ++ " total, " ++
             show (realToFrac fastTime / fromIntegral iterations :: Double) ++ "s/iter"
