      ...
      mkBuck2Project =
        ...
        let
          ...
          shellHook = ''
            ...
            # Generate hie.yaml for HLS
            ${lib.optionalString haskellEnabled ''
                PKG_DB="${ghc}/lib/ghc-${ghcVersion}/lib/package.conf.d"
              ...
              #
              # Features:
              #   - Type information on hover (all packages)
              #   - Documentation on hover (all packages)  
              #   - Completions from all installed packages
              #   - Go-to-definition for project files
              #   - Diagnostics and code actions
              #
              # Limitation: Go-to-definition for external packages not yet supported.
              # This is a known HLS limitation being addressed in PR #3749.
              # For external package source code:
              #   - Use 'hoogle <function>' to search and view docs
              #   - Or 'hoogle server --local -p 8080' for web UI
              #
              cradle:
                direct:
                  arguments:
              ...
              EOF
                ...
                # hoogle is pre-configured with database for project packages
                # Usage: hoogle search <query>
                #        hoogle server --local -p 8080  (web UI)
            ''}
