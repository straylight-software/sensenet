  ...
  config = mk-if cfg.enable {
    "perSystem" =
      ...
      let
        ...
        buckconfig-re-snippet = write-text "buckconfig-re.local" ''
          # ────────────────────────────────────────────────────────────────────────
          ...
          [buck2_re_client]
          engine_address = ${cfg.buck2.engine-address}
          cas_address = ${cfg.buck2.cas-address}
          action_cache_address = ${action-cache-addr}
          tls = ${if cfg.buck2.tls then "true" else "false"}
          instance_name = ${cfg.buck2.instance-name}

          [buck2_re_client.platform_properties]
          OSFamily = ${cfg.buck2.platform-properties.os-family}
          container-image = ${cfg.buck2.platform-properties.container-image}
        '';
